...
#define Pi			3.1415926535897f	// PI already defined by arduino
...
template<typename T>
T mapp(T input, T in_min, T in_max, T out_min, T out_max)	// mapp already defined by arduino
{
	return T((input - in_min) * (out_max - out_min) / (in_max - in_min) + out_min);
}

template<typename T>
T constrainn(T in, T minimum, T maximum)	// constrain already defined by arduino
{
